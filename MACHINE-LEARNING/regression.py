...
# Gradient Decent 梯度下降法 自建 simple linear regression 模型
...
for j in range(repeats):                    # y(x)=wo+w1x
    ...
    error_sum_x = 0                         # w0=w0+alpha*sum(y-y(x))
    for i in range(len(X_train)):           # w1=w1+alpha*sum(y-y(x))*x
...
plt.title("gradient decent regression predict")
plt.legend()
plt.show()


from sklearn.metrics import r2_score  # 引入 R^2 計算工具

# 訓練集預測
predicts_train = w0 + w1 * X_train
# 測試集預測
X_test = np.array(X_test)
y_test = np.array(y_test)
predicts_test = w0 + w1 * X_test

# 計算 R^2 分數
r2_train = r2_score(y_train, predicts_train)  # 計算訓練集的 R^2
r2_test = r2_score(y_test, predicts_test)     # 計算測試集的 R^2

print("訓練集模型準確度", r2_train)
print("測試集模型準確度", r2_test)
